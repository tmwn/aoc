...
#[aocio]
pub fn small(mut prog: Program) -> usize {
    let map = prog.read_ascii(false);
    let h = map.len();
    let w = map[0].len();
    let mut res = 0;
    for i in 0..h {
        for j in 0..w {
            let mut c = 0;
            for (x, y) in grid::neighbors(i, j, h, w).chain([(i, j)]) {
                if map[x][y] == '#' {
                    c += 1;
                }
            }
...
#[aocio]
pub fn large(mut prog: Program) -> i32 {
    let map = prog.read_ascii(true);

    let mut state = State::new(map);
    let mut routines = vec![];
    let mut main = vec![];
    dfs(&mut state, &mut routines, &mut main);

    todo!();
}

fn dfs(state: &mut State, routines: &mut Vec<Routine>, main: &mut Vec<usize>) -> bool {
    for i in 0..routines.len() {
        if state.push_all(&routines[i]) {
            main.push(i);
            if state.remain == 0 {
                return true;
            }
            if dfs(state, routines, main) {
                return true;
            }
            main.pop();
            state.pop(&routines[i]);
        }
    }
    // Add routine
    if routines.len() >= 3 {
...
struct Routine(Vec<Instr>);

impl Routine {
    fn push(&mut self, i: Instr) -> bool {
        if let (Some(Instr::Turn(_)), Instr::Turn(_)) = (self.0.last(), i) {
            return false;
        }
        self.0.push(i);
        if self.len() < 20 {
...
impl State {
    fn new(map_char: Vec<Vec<char>>) -> State {
        let h = map_char.len() + 2;
        let w = map_char[0].len() + 2;

        let mut x = 0;
        let mut y = 0;
        let mut d = 0;
        let mut remain = 0;
        let mut map = vec![vec![i32::MIN; w]; h];
        for i in 1..h - 1 {
            for j in 1..w - 1 {
                match map_char[i - 1][j - 1] {
                    '.' => (),
                    '#' => {
                        map[i][j] = 0;
                        remain += 1;
    ...
    fn is_valid(&self) -> bool {
        ...
        let c = 0;
        for i in 0..self.instrs.len()-1 {
            match (self.instrs[i], self.instrs[i+1]) {
                (Instr::Forward, Instr::Turn(_)) => c += 1,
                (Instr::Turn(_), Instr::Forward) => c += 1,
                _ => (),
            }
        }
        if c > 20 {
    ...
    fn push(&mut self, c: Command) -> bool {
        match c {
            ...
            Command::MakeRutine => todo!(),
            Command::RunRoutine(_) => todo!(),
        }
        match (self.instrs.last(), i) {
            (Some(Instr::Turn(_)), Instr::Turn(_)) => return false,
            (Some(Instr::Forward(_)), Instr::Forward(_)) => return false,
            _ => (),
        }
        todo!()
    }
