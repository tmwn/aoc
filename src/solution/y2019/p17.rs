...
pub fn large(mut a: Vec<i64, ",">) -> i64 {
    ...
    let mut map = prog.read_ascii(false);
    map.pop();
    map.pop();
    ...
    let mut state = State::new(map, x, y, d);
    state.solve();

    let mut res = "".to_string();
    for i in state.instr {
        match i {
            Instr::Turn(1) => res.push('L'),
            Instr::Turn(3) => res.push('R'),
            Instr::Forward(x) => res += &x.to_string(),
            _ => panic!(),
        }
        res.push(',');
    }
    ...
    prog.write_ascii("A,B,A,C,A,B,C,B,C,A\n");
    prog.write_ascii("L,12,R,4,R,4,L,6\n");
    prog.write_ascii("L,12,R,4,R,4,R,12\n");
    prog.write_ascii("L,10,L,6,R,4\n");
    prog.read_ascii(false);
    prog.write_ascii("n\n");
    let a = prog.read_all();
    *a.last().unwrap()
}
...
impl State {
    fn new(map: Vec<Vec<char>>, x: usize, y: usize, d: usize) -> State {
        State {
            x,
            y,
            d,
    ...
    fn try_forward(&mut self) -> bool {
        let x = self.x as isize + grid::DIR4[self.d].0;
        let y = self.y as isize + grid::DIR4[self.d].1;
        if x < 0 || x >= self.map.len() as isize || y < 0 || y >= self.map[0].len() as isize {
            return false;
        }
        if self.map[x as usize][y as usize] == '.' {
            return false;
        }
        self.x = x as usize;
        self.y = y as usize;
        if let Some(Instr::Forward(x)) = self.instr.last_mut() {
            *x += 1;
        } else {
