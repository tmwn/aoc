...
#[aocio]
pub fn small(a: Program) -> i32 {
    solve(a).0
}

#[aocio]
pub fn large(a: Program) -> i32 {
    solve(a).1
}

fn solve(mut prog: Program) -> (i32, i32) {
    let mut map = vec![vec!['?'; N]; N];
    let init_x = N / 2;
    let init_y = N / 2;
    map[init_x][init_y] = '.';
    dfs(init_x, init_y, &mut map, &mut prog);
    ...
    let small = dists_from(init_x, init_y, &map)[ox][oy];

    let dist = dists_from(ox, oy, &map);
    let mut large = 0;
    for i in 0..N {
        for j in 0..N {
            if dist[i][j] < i32::MAX {
                large = large.max(dist[i][j]);
            }
        }
    }
    (small, large)
...
fn dists_from(x: usize, y: usize, map: &Map) -> Vec<Vec<i32>> {
    ...
    while let Some((x, y)) = q.pop_front() {
        for (nx, ny) in grid::neighbors(x, y, N, N) {
            if map[nx][ny] == '#' || dist[nx][ny] < i32::MAX {
                continue;
            }
...
fn dfs(x: usize, y: usize, map: &mut Map, prog: &mut Program) {
    for (nx, ny) in grid::neighbors(x, y, N, N) {
        if map[nx][ny] != '?' {
            continue;
        }
        let d = match ((nx < x, nx > x), (ny < y, ny > y)) {
            ((true, _), _) => 2,
            ((_, true), _) => 1,
            (_, (true, _)) => 3,
            (_, (_, true)) => 4,
